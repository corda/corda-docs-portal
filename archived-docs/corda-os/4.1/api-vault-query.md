...
`CONSUMED`), state reference, contract state type, notary name, soft locked states, timestamps (`RECORDED`, `CONSUMED`), state constraints (see [Constraint Types](api-contract-constraints.html)), and relevancy (`ALL`, `RELEVANT`, `NON_RELEVANT`).
{{< note >}}
