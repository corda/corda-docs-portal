...
new Corda release. The stable Corda modules are listed [here](api-stability-guarantees.md). Our CI process runs an "API Stability" check for each GitHub pull request in order to check that we don’t accidentally introduce an API-breaking change.
