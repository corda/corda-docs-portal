...
* The Corda `.jar` artifact file, downloaded from https://software.r3.com/ui/native/corda-releases/net/corda/corda/4.5/corda-4.5.jar.
...
* `sshdPort` &lt;integer&gt; - use this configuration option to specify the port for sshd communication. For more information, ee [sshd](corda-configuration-fields.html#sshd). For example:
...
To configure [package namespace ownership](../../4.5/enterprise/node/deploy/env-dev.html#package-namespace-ownership), use the optional `networkParameterOverrides` and `packageOwnership` blocks, in a similar way to how the configuration file is used by the [Network Bootstrapper](network-bootstrapper.md) tool. For example:
