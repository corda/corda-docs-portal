...
weight: 60
aliases: /docs/corda-enterprise/4.6/node/operating/cm-updating-cordapp.md
---
...
The general recommendation for Corda 4 is to use **implicit** upgrades for the reasons described [here](cordapps/api-contract-constraints.html#implicit-vs-explicit-contract-upgrades).
...
Follow the guide in [contract constraints](cordapps/api-contract-constraints.md).
...
`QueryableState` interface and creating a custom object relational mapper for the state. See [state persistence](cordapps/state-persistence.md)
for details.
