...
Running the migration at startup automatically can only be configured by using the `initial registration` sub-command when running the node. The standard way of running the schema initialisation / migration scripts is to run the `run-migration-scripts` sub-command - see [Node command-line options](../node-commandline.md).
...
You can review all available commands and options in the [Database Management Tool documentation](../../database-management-tool.md).
...
The structure of the tables of JPA notaries are described at [Configuring a JPA notary backend](../../notary/installing-jpa.html#configuring-a-jpa-notary-backend).
