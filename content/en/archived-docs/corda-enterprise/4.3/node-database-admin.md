---
aliases:
- /releases/4.3/node-database-admin.html
date: '2020-01-08T09:59:25Z'
menu:
  corda-enterprise-4-3:
    identifier: corda-enterprise-4-3-node-database-admin
...
* [Corda node configuration](#3-corda-node-configuration)
...
The permissions for the Corda node user to access database objects will be assigned in [the following step](node-operations-cordapp-deployment.md)
...
Refer to [Corda Database Management Tool](node-database.html#database-management-tool) manual for more detail.
...
Alternatively the path can be also specified by the `jarDirs` option in [the node configuration](corda-configuration-file.html#jardirs).
...
Place the PostgreSQL JDBC Driver *42.2.8* version *JDBC 4.2* in the node directory `drivers`.
