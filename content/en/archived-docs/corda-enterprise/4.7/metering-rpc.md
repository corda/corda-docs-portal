...
{{< note >}}
Destination nodes must be configured to share their metering data with the designated collector node. For more information, see [How metering data is shared](../../../../../en/platform/corda/4.7/enterprise/metering-collector.html#how-metering-data-is-shared).
{{< /note >}}

{{< note >}}
Only the transactions that cause a ledger update are reported (i.e., NORMAL, CONTRACT_UPGRADE, and NOTARY_CHANGE). The transactions that do not cause a ledger update are not included in the metering report. For more information, see [available transaction types](../../../../../en/platform/corda/4.7/enterprise/metering-collector.html#filtering-by-transaction-type).
{{< /note >}}
