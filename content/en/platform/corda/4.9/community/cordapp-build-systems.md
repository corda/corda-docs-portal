...
menu:
  corda-community-4-9:
    identifier: corda-community-4-9-cordapp-build-systems
...
The Cordformation plugin can also sign CorDapp `.jar`s when [deploying a set of nodes]({{< relref "../enterprise/node/deploy/generating-a-node.md" >}}).
...
You can find an example project that demonstrates this in the `samples` folder of the Corda Git repository, `cordapp-configuration` . You can also refer to the [API documentation*****.
...
{{< note >}}
Before you install a CorDapp `.jar`, you must [create one or more nodes]({{< relref "../enterprise/node/deploy/generating-a-node.md" >}}) to install it on.
