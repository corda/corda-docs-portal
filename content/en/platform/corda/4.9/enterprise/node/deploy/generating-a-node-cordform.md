...
To configure [package namespace ownership](env-dev.html#package-namespace-ownership), use the optional `networkParameterOverrides` and `packageOwnership` blocks, in a similar way to how the configuration file is used by the [Network Bootstrapper]({{< relref "../../network-bootstrapper.md" >}}) tool. For example:
...
You can force them to be checked by zone constraints by adding contract class names to the `includeWhitelist` entry - the list will generate an `include_whitelist.txt` file used internally by the [Network Bootstrapper]({{< relref "../../../../../../../en/platform/corda/4.9/enterprise/network-bootstrapper.md" >}}) tool.
Before you add `includeWhitelist` to the `deployNodes` task, see [Contract Constraints]({{< relref "../../../../../../../en/platform/corda/4.9/enterprise/cordapps/api-contract-constraints.md" >}}) to understand the implications of using different constraint types.
