...
* `--pause-all-flows`: Paused all flows when the node starts up. Starting a node with the `--pause-all-flows` command-line option automatically enables flow draining mode but does not modify the node's configuration file. See [Pause and resume flows]({{< relref "../../../../../../../en/platform/corda/4.9/enterprise/flow-pause-and-resume.md#starting-the-node-and-pausing-all-flows" >}}) for more information.
