---
date: '2020-04-07T12:00:00Z'
menu:
  corda-enterprise-4-9:
    identifier: "corda-enterprise-4-9-upgrading-menu"
    name: "Upgrading a CorDapp or Node"
tags:
- upgrading
title: Upgrading a CorDapp or node
weight: 2
---

# Upgrading a CorDapp or node

* [Upgrading a node to Corda 4.9](node-upgrade-notes.md)
* [Upgrading a CorDapp to Corda: Enterprise Edition  4.9](app-upgrade-notes-enterprise.md)
* [Upgrading a CorDapp to a newer platform version](app-upgrade-notes.md)
* [Upgrading a Corda OS Node to Corda: Enterprise Edition ](node-operations-upgrading-os-to-ent.md)
* [Upgrading deployed CorDapps](node-operations-upgrade-cordapps.md)
* [Upgrading a running CorDapp](upgrading-cordapp.md)

## Features, versions, and compatibility

* [Corda and Corda: Enterprise Edition  compatibility](version-compatibility.md)
* [Platform support matrix](platform-support-matrix.md)

## See also

* [Upgrading Corda: Enterprise Edition  Network Manager](../../1.5/cenm/upgrade-notes.md)
