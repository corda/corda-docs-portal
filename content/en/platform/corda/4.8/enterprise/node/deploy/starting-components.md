...
`/usr/bin/java -Xmx1024m -jar /opt/corda/corda-firewall-4.8.jar --config-file float.conf`
...
`/usr/bin/java -Xmx2048m -jar /opt/corda/corda-4.8.jar --config-file node.conf`
...
To do so using the [Configuration Obfuscator](../../../../../../../en/platform/corda/4.8/enterprise/tools-config-obfuscator.md) command-line tool, use the `--config-obfuscation-seed` and `--config-obfuscation-passphrase` flags, respectively, in your node run command.
...
```bash
$ /usr/bin/java -Xmx2048m -jar /opt/corda/corda-4.8.jar --config-file node.conf --config-obfuscation-seed my-seed --config-obfuscation-passphrase my-passphrase
...
```bash
$ export CONFIG_OBFUSCATION_SEED=my-seed; export CONFIG_OBFUSCATION_PASSPHRASE=my-passphrase; /usr/bin/java -Xmx2048m -jar /opt/corda/corda-4.8.jar --config-file node.conf
```
{{< /note >}}
...
* Add the `smmStartMode="Safe"` option to the [node configuration file](../../../../../../../en/platform/corda/4.8/enterprise/node/setup/corda-configuration-file.md).

These flows can then be individually retried via RPC or the node shell.

See [Pause and resume flows](../../../../../../../en/platform/corda/4.8/enterprise/flow-pause-and-resume.html#starting-the-node-and-pausing-all-flows) for more information.
{{< /note >}}
...
`/usr/bin/java -Xmx1024m -jar /opt/corda/corda-firewall-4.8.jar`
