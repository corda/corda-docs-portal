...
3. If you are using the Corda gradle plugin configuration tasks, alter the task to include the checkpoint agent. See [the cordform task](../../../../../../../../en/platform/corda/4.8/enterprise/node/deploy/generating-a-node.md) for information on updating the `cordform` task.
