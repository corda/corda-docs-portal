...
The tool is distributed as part of Corda Enterprise in the form of runnable `.jar.` file - `corda-tools-ha-utilities-4.8.jar`.

To run simply pass in the file or URL as the first parameter:

```kotlin
java -jar corda-tools-ha-utilities-4.8.jar <sub-command> <command line options>
```
...
* `install-shell-extensions`: Install alias and autocompletion for bash and zsh. See ({{< relref "node/operating/cli-application-shell-extensions.md" >}}) for more info.
...
```kotlin
.
├── corda-tools-ha-utilities-4.8.jar
├── drivers
    ├── <HSM driver>
├── entity_A
    ├── node.conf
