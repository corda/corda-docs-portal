...
The notary virtual node selection logic (`NotaryVirtualNodeSelectorServiceImpl`) randomly selects a virtual node associated with a service identity, including pending or suspended members.

### Flow Service Exception

In some instances, the flow service throws an exception if a lambda is used. This issue is caused by a bug in the ASM library.
