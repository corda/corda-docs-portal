...
A new `FindUnconsumedByExactType` function has been added, which replaces `findUnconsumedStatesByType`. The performance of this new function is greater than `FindUnconsumedByType` because it uses the index search that retrieves the given exact-type states (not including subclass states).
