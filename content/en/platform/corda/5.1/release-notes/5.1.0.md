...
For information about upgrading a Corda Cluster from 5.0 to 5.1, see []({{< relref "../deploying-operating/deployment/upgrading/_index.md" >}}).
...
A new interface, `GroupParametersLookup`, has been added to the `net.corda.v5.membership` package to enable flows to access `GroupParameters` for further verification.

#### findUnconsumedStatesByType Performance

The performance of the retrieval of unconsumed states in the `findUnconsumedStatesByType` query has been improved. As a result, it it is now more efficient to scan the table with an index on `utxo_visible_transaction_state.consumed`.

#### FindUnconsumedByExactType

A new `FindUnconsumedByExactType` function has been added. The performance of this new function is greater than `FindUnconsumedByType` because it uses the index search that retrieves the given exact-type states (not including subclass states).

#### Verification

Verification functionality has been added to a new `TransactionSignatureVerificationService` interface. This enables it to be injected separately in the verification sandbox.

#### Notary Signed Group Parameters

Signed group parameters are now distributed via backchain resolution to allow notary verification for historic transactions.
...
Old endpoints from v1 work in 5.1 but a warning will be written to the REST Worker log and in the HTTP response.
...
* The `members` endpoint now returns the `MemberInfo` for the member on behalf of who the lookup is performed even if the member has a status of suspended. This ensures that a member is able to check their status at any point in time.
...
* A new endpoint has been added that returns the result of a flow as JSON.
