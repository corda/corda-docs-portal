...
menu:
  corda-enterprise-4-6:
    identifier: corda-enterprise-4-6-operations-guide-deployment-node-generating
...
* `name` <string>
    * The legal identity name of the Corda node. (see [myLegalName](../../node/setup/corda-configuration-file.md))
    * e.g.
...
* `p2pAddress` <string> <**required if p2pPort not specified**>
    * The address/port the node uses for inbound communication from other nodes. (see [p2pAddress](../../node/setup/corda-configuration-file.md))
    * e.g.
...
* `p2pPort` <integer>
    * The port the node uses for inbound communication from other nodes.  Assumes the address is `localhost`. (see [p2pAddress](../../node/setup/corda-configuration-file.md))
    * e.g.
...
* `rpcSettings` <config>
    * Specifies RPC settings for the node. (see [rpcSettings](../../node/setup/corda-configuration-file.md))
    * e.g.
...
* `notary` <config> (see [notary](../../node/setup/corda-configuration-file.md))
...
* `devMode` <boolean>
    * When true enables development mode. (see [devMode](../../node/setup/corda-configuration-file.md))
    * e.g.
...
* `webPort` <integer>
    * Configure a webserver to connect to the node via RPC. Defaults the address to *localhost*. The node must have an RPC address configured. (see [Specifying a custom webserver](#specifying-a-custom-webserver))
    * e.g.
...
* `rpcUsers` <list>
    * Set the RPC users for this node. (see [rpcUsers](../../node/setup/corda-configuration-file.md))
    * e.g.
...
* `sshdPort` <integer>
    * Specifies the port for sshd communication. (see [sshd](../../node/setup/corda-configuration-file.md))
    * e.g.
