...
The two APIs are available from `CordaRPCOps` (see [Using the client RPC API](../../../../../en/platform/corda/4.6/open-source/tutorial-clientrpc-api.md)). They both return a `FlowHandleWithClientId`, which extends `FlowHandle` but also contains the `clientId` that the flow started with.
