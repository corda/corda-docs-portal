...
The registering member must then include this pre-auth {{< tooltip >}}token{{< /tooltip >}} in the registration request they submit when registering. To do this, an additional key must be set in the registration context. This key is `corda.auth.token`, and the value of this key must be the pre-auth token that the MGM provided.
