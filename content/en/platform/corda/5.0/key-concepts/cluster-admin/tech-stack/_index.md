...
All of the above keys are stored in the Crypto databases (cluster and virtual nodes) and they are all encrypted at rest with “Wrapping keys”. 
...
As indicated on the diagram, key wrapping is hierarchical. The master wrapping key protects other wrapping keys, such as the virtual node related keys, which in turn protect the private keys used by Corda. It must never be possible for someone with a copy of the Corda database or a virtual node database to decrypt the keys stored in the database using other information stored in the database and so, the master wrapping key, or the information required to generate this key, must be stored and managed outside Corda. This can be acheived in one of the following ways:
...
* {{< enterprise-icon >}} A key can be stored and managed by an external key management system and picked up by Corda when required.
