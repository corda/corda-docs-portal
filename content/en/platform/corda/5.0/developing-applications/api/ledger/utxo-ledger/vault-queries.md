...
* The implementation of the JSON factory must be defined in the same {{< tooltip >}}CPK{{< /tooltip >}} as the state that it is working on, so that the platform can get hold of it when persisting a state to the database.
{{< /note >}}
