...
1. Retrieve the existing `IdentifiableState`, `id` or its `StateRef` if the `id` is `null`.
2. Create a new instance of the `IdentifiableState` and pass in the `id` or `StateRef` from the previous step.
3. Consume the existing instance in the same transaction that contains the updated instance.
