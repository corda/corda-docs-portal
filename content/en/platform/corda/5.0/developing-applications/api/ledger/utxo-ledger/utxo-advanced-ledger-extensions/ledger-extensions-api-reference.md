...
2. Extract the `id` or `StateRef` from the state once it has been retrieved, only use the `StateRef` if the `id` is `null` which occurs when it has been updated for the first time
