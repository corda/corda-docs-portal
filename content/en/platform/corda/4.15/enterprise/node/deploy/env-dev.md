...
menu:
  corda-enterprise-4-15:
    identifier: corda-enterprise-4-15-corda-nodes-deploying
...
The Corda Network Bootstrapper can be downloaded [here](https://download.corda.net/maven/corda-releases/net/corda/corda-tools-network-bootstrapper/4.15/corda-tools-network-bootstrapper-4.15.jar).
...
```bash
java -jar corda-tools-network-bootstrapper-4.15.jar --dir <nodes-root-dir>
```
...
```bash
java -jar corda-tools-network-bootstrapper-4.15.jar --network-parameter-overrides=<path_to_file>
```

Or alternatively, by using the short form version:

```bash
java -jar corda-tools-network-bootstrapper-4.15.jar -n=<path_to_file>
```
...
variable `KEY_STORE_PASSWORD`. See [Hiding sensitive data]({{< relref "../operating/node-administration.md#hiding-sensitive-data" >}}).
