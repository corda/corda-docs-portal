...
`CONSUMED`), state reference, contract state type, notary name, soft locked states, timestamps (`RECORDED`, `CONSUMED`), state constraints (see [Types of Contract Constraints](../../../../../../../en/platform/corda/4.5/enterprise/cordapps/api-contract-constraints.html#types-of-contract-constraints)), relevancy (`ALL`, `RELEVANT`, `NON_RELEVANT`), and participants (exact or any match).
{{< note >}}
