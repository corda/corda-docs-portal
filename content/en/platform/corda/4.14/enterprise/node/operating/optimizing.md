...
Flow and RPC threads can be set explicitly using the [tuning]({{< relref "../setup/corda-configuration-fields.md#tuning" >}}) element of the `enterpriseConfiguration` field. Add the following section to your node configuration file:
...
Therefore, a standard, read-optimised database as is, for example, on offer from cloud providers does not suit the performance requirements of Corda very well. R3 recommends to run a dedicated database on a server that has fast disks, so the writing of checkpoints does not slow the processing down.
...
Corda uses TLS to encrypt the peer to peer connections between nodes. It can be shown that the maximal throughput that is achievable with the JVM TLS layer can limit the node throughput for flows that need to send sufficient amount of data between peers. This is, for example, the case for the *CashIssueAndPayment* flow in the performance test CorDapp.
