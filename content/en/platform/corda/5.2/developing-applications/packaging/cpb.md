...
2. To sign your CPB, add a `cordapp/signing` section to the project `settings.gradle` file.
   ...
   {{< note >}}
By default, the CPB plugin signs with a default key. This key should only be used for local testing. You can deploy locally on a static network, using the [CSDE]({{< relref "../runtime-plugin/_index.md">}}).
