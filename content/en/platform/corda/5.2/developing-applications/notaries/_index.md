...
This section outlines how to configure a [notary]({{< relref "../ledger/notaries/_index.md" >}}) for a Corda {{< tooltip >}}application network{{< /tooltip >}}. It contains the following:
...
Notary functionality is provided in the form of plugin CorDapps. In theory, anyone can write a new notary protocol by implementing their own CorDapps. However, initially, it is expected to only use protocols provided by R3. For a given notary protocol, two {{< tooltip >}}CPBs{{< /tooltip >}} are expected to be required:
...
For {{< version >}}, the following notary protocols are available:
...
Each protocol is composed of multiple modules/CPKs, which are packaged into two different CPBs. The following module is common to all protocols:
...
The following sections describe the modules specific to each protocol:

* [Contract-Verifying Protocol Modules](#contract-verifying-protocol-modules)
* [Non-validating Notary Protocol Modules](#non-validating-notary-protocol-modules)

#### Contract-Verifying Protocol Modules

* **`notary-plugin-contract-verifying-api`** —  
* **`notary-plugin-contract-verifying-client`** —  
* **`notary-plugin-contract-verifying-server`** —  

The source code for these modules can be found in the [notary-plugin-contract-verifying](https://github.com/corda/corda-runtime-os/tree/release/os/5.2/notary-plugins/notary-plugin-contract-verifying) folder in the [notary-plugins area of corda-runtime-os](https://github.com/corda/corda-runtime-os/tree/release/os/5.2/notary-plugins).

#### Non-validating Notary Protocol Modules
...
* **`non-validating-notary-server`** —  provides a responder flow to the corresponding initiator flow in the client package, and performs notarization processing.

The source code for these modules can be found in the [notary-plugin-non-validating](https://github.com/corda/corda-runtime-os/tree/release/os/5.2/notary-plugins/notary-plugin-non-validating) folder in the [notary-plugins area of corda-runtime-os](https://github.com/corda/corda-runtime-os/tree/release/os/5.2/notary-plugins).
...
*************** you must bundle the `notary-common`, `<notary-type>-api`, and `<notary-type>-client` CPKs when creating your CPB. For the contract-verifying notary to work, you must package workflows and contracts in separate CPKs.
