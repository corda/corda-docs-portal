...
#### Contract-Verifying Notary

It is now possible to implement a contract-verifying notary. This enables you to create a network with a trusted notary that can verify the validity of transactions. Therefore, full resolution and verification of the whole backchain can be replaced by a check that verifies that the inputs of a transaction come from a transaction signed by the verifying notary, removing the requirement to resolve an ever growing backchain.

For more information, see [Notaries]({{< relref "../developing-applications/notaries/_index.md" >}}).
