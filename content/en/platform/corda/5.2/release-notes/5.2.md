...
It is now possible to create a network with a trusted notary that can verify the validity of transactions.
A notary that knows about the contracts governing the transactions can verify that all transactions adhere to these contracts before signing them. The notary must check that any inputs to a transaction it verifies come from a transaction that has been signed by the same notary service. For improved efficiency and privacy, the inputs to a proposed transaction can be sent as filtered transactions to only reveal the relevant parts of a transaction.
This can replace full resolution and verification of the whole backchain, removing the requirement to resolve an ever growing backchain.

For more information, see [Notaries]({{< relref "../developing-applications/ledger/enhanced-ledger-privacy.md" >}}).
