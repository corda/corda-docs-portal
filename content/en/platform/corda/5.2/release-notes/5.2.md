...
The ledger repair functionality checks each member’s vault for transactions that might have been notarized, sends any such transactions to the notary to check whether they were previously seen, and updates the member’s vault if the transactions were notarized. By default, Corda runs the ledger repair process every ten minutes. You can also run a check manually using the new `com.r3.corda.notary.plugin.common.repair.NotarizedTransactionRepairFlow` flow.
