...
For convenience, these two flows have initiating flows available in `com.r3.corda.lib.ci.workflows.InitiatingFlows` which can be called as subflows instead of initating sessions in a custom flow.
...
`RequestKeyFlow` takes as its constructor arguments the counter-party session we want to exchange confidential identities with initiated by one of the above mentioned three flows. It returns an anonymous party representing the counter-party. The three initiating flows mentioned above also return this same anonymous party.
...
Changes to the way confidential identities are handled have prompted updates to some elements of the Indentity Service API. The core concepts have not changed—it is still a directory of parties, with their names and public keys.
...
-   The `partiesFromName` method has been removed. You can acheive similar RPC functionality with the [member lookup service](xxx).
