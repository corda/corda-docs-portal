...
This tool enables the reissuing of node legal identity keys and certificates, allowing for re-registration of a node (including a notary node) with a new certificate in the Network Map in {{< cenmlatestrelref "cenm/_index.md" "Corda Enterprise Network Manager" >}}. You must not change the node's `myLegalName` during certificate rotation.
