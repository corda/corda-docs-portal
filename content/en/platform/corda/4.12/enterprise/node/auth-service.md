...
1. Create a SSL certificate in a `.jks` file using the {{< cenmlatestrelref "cenm/pki-tool.md" "CENM PKI tool" >}}.
...
{{<note>}}
If multiple CENM instances are connected to the same database, setting `lockResolutionStrategy` to `SingleInstance` can cause startup problems and/or database corruption. For more information, see the {{< cenmlatestrelref "cenm/config-database.md" "database configuration options" >}}
{{</note>}}
