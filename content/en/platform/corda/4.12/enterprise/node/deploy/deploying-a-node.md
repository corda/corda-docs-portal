...
* Place the Enterprise Corda JAR `corda-4.12.jar` in `/opt/corda`
...
* Create a Corda directory and copy the Enterprise Corda JAR `corda-4.12.jar`.
Here’s an example using PowerShell:

```shell
mkdir C:\Corda
copy PATH_TO_CORDA_JAR/corda-4.12.jar C:\Corda\corda.jar
```
