...
As mentioned in the description of the configuration file ([Node configuration](corda-configuration-file.md)), the `node.conf` has three relevant fields:
* `cryptoServiceName`
* `cryptoServiceConf`
...
The Azure key vault client jar needs to be placed in the driver's directory.  The dependent jars are azure-keyvault and adal4j.
