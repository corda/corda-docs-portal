...
By default, the private keys that belong to the node CA, legal identity and TLS are stored in key store files in the node’s
certificates directory. Users may wish to instead store this key in a hardware security module (HSM) or similar.

See the [Hardware Security Modules (HSM)](../../../../../../../en/platform/corda/4.7/enterprise/platform-support-matrix.html#hardware-security-modules-hsm) for supported HSMs and their features.
