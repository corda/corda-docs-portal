...
receiver flows to use the new APIs and thus opting in to platform version 4. See [Step 5. Security: Upgrade your use of FinalityFlow](app-upgrade-notes.md#step-5-security-upgrade-your-use-of-finalityflow) for more information.
...
that requires programmatic code changes. See [CZ whitelisted constraints migration](cordapp-constraint-migration.md#migrating-cz-whitelisted-constraints) for more information.
* Explicit Contract Upgrades are only supported for hash and CZ whitelisted constraint types. See [Performing explicit contract and state upgrades](upgrading-cordapps.md#performing-explicit-contract-and-state-upgrades) for more information.
...
See [Installing the CorDapp JAR](cordapp-build-systems.md#installing-the-cordapp-jar) and [CorDapp Contract Attachments](cordapp-build-systems.md#cordapp-contract-attachments) for more information.
...
where the latest installed contract Jar is signed as per [CorDapp Jar signing](cordapp-build-systems.md#signing-the-cordapp-jar).
