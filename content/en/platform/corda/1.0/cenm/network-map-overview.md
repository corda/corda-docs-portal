---
aliases:
- /releases/release-1.0/network-map-overview.html
date: '2020-01-08T09:59:25Z'
menu:
  cenm-1-0:
    identifier: cenm-1-0-network-map-overview
...
If the node is configured to connect with a compatibility zone using the `networkServices` section of the config, then it first uploads its own signed `NodeInfo`
...
|GET|/network-map-user/network-map/{uuid}|Retrieve the current network map for a private network indicated by the uuid parameter formatted as a JSON document.|
...
|GET|/network-map-user/node-infos/{uid}|Retrieve a human readable list of the currently registered `NodeInfo` files in the specified private network map.|
...
more about *contract constraints* in the [contract constraints doc](../../../../../en/platform/corda/4.5/open-source/api-contract-constraints.html#types-of-contract-constraints). See
...
Private networks are not separate networks, nodes are still part of bigger network, only hidden. We reuse all the
infrastructure of the network like notaries, permissioning service, so the interoperability between nodes is kept.

An additional REST `/network-map/{uuid}` endpoint serving private network maps was introduced. For nodes to be able to query
that information automatically you need to change `node.conf` to include private network UUIDs in `extraNetworkMapKeys`.

See [Private Network Map](private-network-map.md) for more information and steps for setting up a private network.
