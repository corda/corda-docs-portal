...
menu:
  corda53:
    identifier: corda53-api-ledger-utxo-token-selection
...
* **Token Priority:** Some scenarios require tokens to have a specified priority, so they get consumed first. For example, when tokens represent vouchers with expiry dates, it is important to always consume the tokens that are expiring first.
...
| Priority         | Long             | User        | An optional long integer that defines the token's consumption priority, with lower values indicating the highest priority. The `null` value sets the priority to its lowest. |
...
Tokens can be claimed using two strategies. The default `RANDOM` strategy selects tokens arbitrarily, while the `PRIORITY` strategy claims the tokens with the highest priority first.
