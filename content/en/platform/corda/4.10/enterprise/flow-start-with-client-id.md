...
The two APIs are available from `CordaRPCOps` (see [Using the client RPC API](../../../../tutorials/corda/4.10/community/supplementary-tutorials/tutorial-clientrpc-api.md)). They both return a `FlowHandleWithClientId`, which extends `FlowHandle` but also contains the `clientId` that the flow started with.
...
{{< note >}}
When you use this API function, the associated flow must be initiated by the same RPC user that is invoking this method to reattach to the flow. This method only works in scope of the same RPC user that initiated the flow. Only admins can remove client `id` mappings of any user.
{{</ note >}}
