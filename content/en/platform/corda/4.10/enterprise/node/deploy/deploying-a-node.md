...
* Save the following as `/opt/corda/node.conf`. See [Node configuration]({{< relref "../setup/corda-configuration-file.md" >}}) for a description of these options:
...
* Create a Corda directory and copy the Enterprise Corda JAR `corda-4.10.jar`.
Here’s an example using PowerShell:

```shell
mkdir C:\Corda
copy PATH_TO_CORDA_JAR/corda-4.10.jar C:\Corda\corda.jar
```
...
* Depending on the versions of Corda and of the CorDapps used, database migration scripts might need to run before a node is able to start. For more information, see [Database management scripts]({{< relref "../../cordapps/database-management.md" >}}).
