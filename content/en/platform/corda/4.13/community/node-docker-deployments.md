...
menu:
  corda-community-4-13:
    identifier: corda-community-4-13-node-docker-deployments
...
The requirements to perform this are a valid `node.conf` and a valid set of certificates signed by the CZ.
...
The requirements to perform this are a valid `node.conf`, a valid set of certificates, and an existing `network-parameters` file.
...
The requirements to perform this are a compatibility zone, the Zone Trust Root, and authorization to join the zone.

It is possible to use the image to automate the process of joining an existing zone as described in [What is a compatibility zone?]({{< relref "compatibility-zones.md" >}}).
