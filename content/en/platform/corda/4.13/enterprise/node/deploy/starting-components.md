...
`/usr/bin/java -Xmx1024m -jar /opt/corda/corda-firewall-4.13.jar --config-file float.conf`
...
`/usr/bin/java -Xmx2048m -jar /opt/corda/corda-4.13.jar --config-file node.conf`
...
```bash
$ /usr/bin/java -Xmx2048m -jar /opt/corda/corda-4.13.jar --config-file node.conf --config-obfuscation-seed my-seed --config-obfuscation-passphrase my-passphrase
...
```bash
$ export CONFIG_OBFUSCATION_SEED=my-seed; export CONFIG_OBFUSCATION_PASSPHRASE=my-passphrase; /usr/bin/java -Xmx2048m -jar /opt/corda/corda-4.13.jar --config-file node.conf
```
{{< /note >}}

{{< note >}}
If a node is registered but not started and the {{< cenmlatestrelref "cenm/network-map.md#network-map-service" "Network Map Service" >}} goes down before the first start of the node, the node will fail to restart.
{{< /note >}}
...
`/usr/bin/java -Xmx1024m -jar /opt/corda/corda-firewall-4.13.jar`
