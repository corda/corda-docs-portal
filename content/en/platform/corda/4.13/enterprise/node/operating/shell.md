...
```
 java -jar corda-standalone-shell-4.13.jar [-hvV] [--logging-level=<loggingLevel>] [--password=<password>]
...
* [Download](https://download.corda.net/maven/corda-releases/net/corda/corda-shell/4.13/corda-shell-4.13.jar) the `corda-shell` JAR from Maven and install it in a node's `/drivers` directory to run the shell in the same terminal that starts the node.
