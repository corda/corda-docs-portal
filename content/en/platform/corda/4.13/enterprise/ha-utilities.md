...
The tool is distributed as part of Corda Enterprise in the form of runnable `.jar.` file - `corda-tools-ha-utilities-4.13.jar`.

To run simply pass in the file or URL as the first parameter:

```kotlin
java -jar corda-tools-ha-utilities-4.13.jar <sub-command> <command line options>
```
...
* `node-registration`: Corda registration tool for registering 1 or more node with a Corda network, using provided node configuration.
...
The following table shows keys and certificates generated by the tool. For more details on a certificate hierarchy, see [Network certificates]({{< relref "network/permissioning.md" >}}).
...
<a name="ha-utilities-id1"></a><sup>1</sup>
Certificate validity cannot exceed the validity of Doorman CA certificate.<br>
<a name="ha-utilities-id2"></a><sup>2</sup>
For a node TLS certificate, it is recommended to set 27 months validity by specifying <br>`--tls-cert-validity 825`.<br>
<a name="ha-utilities-id3"></a><sup>3</sup>
HSM for TLS keys is only supported with standalone bridge component. TLS keys for ‘all-in-one’ node can be only stored in file.<br>
...
```kotlin
.
├── corda-tools-ha-utilities-4.13.jar
├── drivers
    ├── <HSM driver>
├── entity_A
    ├── node.conf
...
* `-v`, `--verbose`, `--log-to-console`: If set, prints logging to the console as well as to a file.
* `--logging-level=<loggingLevel>`: Enable logging at this level and higher. Possible values: ERROR, WARN, INFO, DEBUG, TRACE. Default: INFO
* `-p`, `--keystore-password=<keyStorePassword>`: Password <sup>[\[4\]](#ha-utilities-id4)</sup> for all generated keystores. Default: changeit
* `-t`, `--truststore-password=<trustStorePassword>`: Password <sup>[\[4\]](#ha-utilities-id4</sup> for the trust store. Default: changeit
...
The name of the HSM provider to be used; for example, `UTIMACO` or `GEMALTO_LUNA`.
...
The name of HSM provider to be used; for example, `UTIMACO` or `GEMALTO_LUNA`.
...
* `--keystore-password`: The SSL keystore password <sup>[\[4\]](#ha-utilities-id4)</sup>.
* `--truststore`: The SSL truststore path.
* `--truststore-password`: The SSL truststore password <sup>[\[4\]](#ha-utilities-id4)</sup>.
...
<a name="ha-utilities-id4"></a>* `--connectors`: A list of network hosts and ports representing the Artemis connectors used for the Artemis HA cluster. The first entry in the list will be used by the instance configured with this tool. The connector entries are separated by commas (for example, localhost:10000,localhost:10001,localhost:10002)
...
<sup>4</sup> The ampersand (&) character is not allowed in the password for Artemis SSL keystore or truststore.
