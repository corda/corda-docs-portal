...
Access the User Admin tool from your [Gateway Service]({{< relref "../../4.10/enterprise/node/gateway-service.md" >}}) instance. Enter the full address of your Gateway Service, including the port number, followed by `/admin` into a web browser.
...
Your initialisation credentials for logging in for the first time are established using the `--initial-user-name` and `--initial-user-password` commands when managing the configuration of the [Auth Service]({{< relref "../../4.10/enterprise/node/auth-service.md" >}}).
