...
![flow interface](/en/images/flow-interface.png "flow interface")In the diagram above, the `InitiatingFlow`:
...
A node can be drained or undrained via RPC using the `setFlowsDrainingModeEnabled` method, and via the shell using
                    the standard `run` command to invoke the RPC. See [Shell](shell.md) to learn more.
...
{{< warning >}}
Deleting checkpoints manually can lead to an inconsistent ledger among transacting parties. Great care
...
{{< note >}}
Beware of the possible classpath clashes! If you keep the old JAR, make sure new one does not contain classes
