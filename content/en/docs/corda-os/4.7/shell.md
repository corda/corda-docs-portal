...
The shell can be used to query flow data. For more information on the types of data that can be queried and instructions for doing so, see the documentation on [Querying flow data](../../corda-enterprise/4.7/node/operating/querying-flow-data.html#querying-flow-data-via-the-node-shell).
