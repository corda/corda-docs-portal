...
This class also provides access to the `mockInjectables` function using to mock the injectable services of a flow if the test author decides they don't need all of `FlowMockHelper` but instead only need a helper to set up the injectable mocks.
