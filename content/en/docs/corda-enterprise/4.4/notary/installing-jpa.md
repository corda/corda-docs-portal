...
Prior to using the JPA notary, the database must be prepared. This can be performed using the
            [Corda Database Management Tool]({{< relref "../node/operating/node-database#database-management-tool-ref" >}}). If preferred, the required tables can be manually
...
The JPA notary uses the Java Persistence API (JPA) interface to connect to the notary state database. For performance
                ...
                listed in the [Platform support matrix]({{< relref "../platform-support-matrix" >}}).
...
{{/* /en/docs/corda-enterprise/4.4/docs/source/notary/resources/dbm.conf */}}
{{< /tabs >}}
...
If the Corda Database Management Tool’s `dry-run` mode is used, the `databasechangelog` and `databasechangeloglock` tables must already exist
                        ...
                        schema modification rights. For more information, see [Corda Database Management Tool]({{< relref "../node/operating/node-database#database-management-tool-ref" >}}).
...
```
Refer to the section [Configuring the notary worker nodes]({{< relref "installing-the-notary-service" >}}) for more details on configuring the JPA notary.
