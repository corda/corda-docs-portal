...
called `remote_hosts`, or can be passed in with the JMeter argument `-R`:

`jmeter.properties``remote_hosts=node1.mydomain.com,node2.mydomain.com``java -jar jmeter-corda.jar -- -Rnode1.mydomain.com,node2.mydomain.com`
