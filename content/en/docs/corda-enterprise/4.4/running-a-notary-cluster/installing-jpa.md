...
Prior to using the JPA notary, the database must be prepared. This can be performed using the
            [Corda Database Management Tool]({{< relref "../node-database#database-management-tool-ref" >}}). If preferred, the required tables can be manually
...
{{/* /en/docs/corda-enterprise/4.4/docs/source/running-a-notary-cluster/resources/dbm.conf */}}
{{< /tabs >}}
...
If the Corda Database Management Tool’s `dry-run` mode is used, the `databasechangelog` and `databasechangeloglock` tables must already exist
                        ...
                        schema modification rights. For more information, see [Corda Database Management Tool]({{< relref "../node-database#database-management-tool-ref" >}}).
