---
aliases:
- /releases/4.4/running-a-notary-cluster/ha-notary-service-setup.html
...
* The relevant HSM library `.JAR` (if storing keys inside a HSM). See [HSM support for legal identity keys](../cryptoservice-configuration.md) for more information.
...
must share the same `notary.serviceLegalName`. For more information, see the [Node configuration](../corda-configuration-file.md).
