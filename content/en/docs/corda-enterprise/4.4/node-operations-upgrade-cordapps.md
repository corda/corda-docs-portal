...
For developer information on upgrading CorDapps, see [Release new CorDapp versions]({{< relref "upgrading-cordapps" >}}).

To be compatible with Corda Enterprise, CorDapps need to bundle database migaration scripts (see [Database management scripts]({{< relref "database-management" >}})).


## Flow upgrades
If any backwards-incompatible changes have been made (see [What constitutes a non-backwards compatible flow change?]({{< relref "upgrading-cordapps#upgrading-cordapps-backwards-incompatible-flow-changes" >}})
...
* *Implicit:* By allowing multiple implementations of the contract ahead of time, using constraints. See
                        [API: Contract Constraints]({{< relref "api-contract-constraints" >}}) to learn more.
...
* If required, do a flow drain to avoid the definition of states or contracts changing whilst a flow is in progress (see [Flow drains]({{< relref "upgrading-cordapps#upgrading-cordapps-flow-drains" >}}) for more information)
