...
The following table shows keys and certificates generated by the tool. For more details on a certificate hierarchy see [Network certificates]({{< relref "permissioning" >}}).
...
This key store contains the service identity certificate (and key if not using a HSM) that all notary workers of this notary cluster share.
                        ...
                        contain the notary service certificate chain. See [HSM Support]({{< relref "running-a-notary-cluster/hsm-support#hsm-support" >}}) for more information.
