...
Run the ledger health check tool before accepting the Corda Enterprise 4.6.3 patch release to detect whether your notary cluster has been affected.
...
```
java -jar tools-ledger-health-check-4.6.3
```
...
If no issues are found, you may safely proceed with applying the 4.6.3 patch. If issues are found contact R3, who will provide further instruction.

After accepting the 4.6.3 patch, you must re-run the Corda Database Management Tool to apply the security fix.
