...
* Create a Corda directory and copy the Enterprise Corda JAR `corda-4.7.jar`.
Here’s an example using PowerShell:

```shell
mkdir C:\Corda
copy PATH_TO_CORDA_JAR/corda-4.7.jar C:\Corda\corda.jar
```
...
From Corda 4.7, the database schema migration process requires you to explicitly perform the following actions. This step is only required when upgrading to Corda 4.7 from a previous version.
