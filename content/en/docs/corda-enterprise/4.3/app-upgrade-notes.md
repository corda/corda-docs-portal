...
These notes provide instructions for upgrading your CorDapps from previous versions. Corda provides backwards compatibility for public,
            non-experimental APIs that have been committed to. A list can be found in the [API stability guarantees]({{< relref "api-stability-guarantees" >}}) page.
...
The issue here is that a new `Destination` interface introduced in Platform Version 5 can cause type inference failures when a variable is
                    ...
                    (For more information on `Destination`, see the [Changelog]({{< relref "changelog" >}}) for Platform Version 5, or the KDocs for the interface
...
Target versioning is a new concept introduced in Corda 4. Learn more by reading [Versioning]({{< relref "versioning" >}}).
...
```
See [CorDapp configuration files]({{< relref "cordapp-build-systems#cordapp-configuration-files-ref" >}}) for more information.
...
{{/* /en/docs/corda-enterprise/4.3/docs/source/example-code/src/main/kotlin/net/corda/docs/kotlin/FinalityFlowMigration.kt */}}
...
{{/* /en/docs/corda-enterprise/4.3/docs/source/example-code/src/main/java/net/corda/docs/java/FinalityFlowMigration.java */}}
{{< /tabs >}}
...
The [Confidential identities]({{< relref "api-identity#confidential-identities-ref" >}}) API is experimental in Corda 3 and remains so in Corda 4. In this release, the `SwapIdentitiesFlow`
...
Learn more by reading [Contract/State Agreement]({{< relref "api-contract-constraints#contract-state-agreement" >}}). If an app targets Corda 3 or lower (i.e. does not specify a target version),
...
[Signature Constraints]({{< relref "api-contract-constraints#signature-constraints" >}}) are a new data model feature introduced in Corda 4. They make it much easier to
...
You can read more about signature constraints and what they do in [API: Contract Constraints]({{< relref "api-contract-constraints" >}}). The `TransactionBuilder` class will
                    ...
                    [Signing the CorDapp JAR]({{< relref "cordapp-build-systems#cordapp-build-system-signing-cordapp-jar-ref" >}}). In dev mode, all JARs are signed by developer certificates. If a JAR that was signed
...
{{< note >}}
Please read the [CorDapp constraints migration]({{< relref "cordapp-constraint-migration" >}}) guide to understand how to upgrade CorDapps to use Corda 4 signature constraints and consume
...
**Namespace ownership.** This part is only relevant if you are joining a production compatibility zone. You may wish to contact your zone operator
                    ...
                    “[Package namespace ownership]({{< relref "network-bootstrapper#package-namespace-ownership" >}})”.
...
In Corda 4 queries made on a node’s vault can filter by the relevancy of those states to the node. As this functionality does not exist in
                    ...
                    to the node in question to query for only relevant states. See [API: Vault Query]({{< relref "api-vault-query" >}}) for more details on how to do this. Not doing this
                    may result in queries returning more states than expected if the node is using observer functionality (see “[Observer nodes]({{< relref "tutorial-observer-nodes" >}})”).
...
* [Reference States]({{< relref "api-states#reference-states" >}}), that let you use an input state without consuming it.


* [State Pointers]({{< relref "api-states#state-pointers" >}}), that make it easier to ‘point’ to one state from another and follow the latest version of a linear state.


Please also read the [CorDapp Upgradeability Guarantees]({{< relref "cordapp-upgradeability" >}}) associated with CorDapp upgrading.
...
Instructions for both options can be found in [Running tests in Intellij]({{< relref "tutorial-cordapp#tutorial-cordapp-running-tests-intellij" >}}).
