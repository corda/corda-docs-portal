---
aliases:
- /releases/4.1/tools-config-obfuscator.html
date: '2020-01-08T09:59:25Z'
menu:
  corda-enterprise-4-1:
    identifier: corda-enterprise-4-1-tools-config-obfuscator
    parent: corda-enterprise-4-1-tools-index-enterprise
    weight: 1030
tags:
- tools
- config
...
* `HARDWARE_ADDRESS` is the primary hardware address of the machine on which the configuration file resides. By default, the MAC address of the running machine will be used. Supplying `DEFAULT` will explicitly use the default value.
* `SEED` is the byte array seeding the encryption key used for obfuscation. Leave blank or supply `DEFAULT` to use the default seed bytes.
